...
| M3. Flight recorder on Claude Code | `hypher` bridge and outbox; Claude Code plugin hooks; session-start injection including after compaction; seal on stop; `hypher init --repo` kit so cloud sessions get the same hooks; a "Hypher unavailable" notice when the network blocks it | Kill the session mid-task; a new session resumes with zero commands; checkpoint under 5 minutes old |
...
| M5. Install and approve | `npx hypher init` and uninstall, keychain credential, one-tap approval, handoff card inside the Pulse writeback row, API keys scoped to one project for cloud environments | A pilot installs on their own repository in under 2 minutes |
