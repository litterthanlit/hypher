...
## Doors: how every agent reaches Hypher

One core, thin kits, no per-tool extension. The hosted MCP server and web API are the core. Every door is configuration around them.

```text
                ┌──────────────────────────────────────┐
                │  Hypher hosted MCP server + web API   │  ← the one core
                └──────────────────────────────────────┘
                     ▲                 ▲              ▲
          local machine           cloud agent       anywhere else
   ┌──────────────────────┐  ┌──────────────────┐  ┌──────────────┐
   │ hypher bridge         │  │ repo kit +       │  │ plain `hypher`│
   │ (optional: outbox,    │  │ env secret       │  │ CLI over the  │
   │  keychain, approvals) │  │ (nothing to      │  │ web API       │
   │                       │  │  install)        │  │               │
   └──────────────────────┘  └──────────────────┘  └──────────────┘
```

| Door | Setup | Credential | Automatic? |
|---|---|---|---|
| Claude Code, local | `npx hypher init` installs the plugin: MCP entry, hooks, commands | OS keychain, shared with hooks through the bridge | Yes (M3) |
| Codex CLI, local | `npx hypher init` writes user-level MCP config, skills, and the `notify` script | Same | Load instructed by `AGENTS.md`; checkpoints via `notify` (M4) |
| Cursor, local | Existing plugin | Existing | Yes, where its hooks run |
| Claude Code on the web | `hypher init --repo` commits `.mcp.json`, `.claude/settings.json` hooks, and `AGENTS.md`/`CLAUDE.md` | `HYPHER_API_KEY` as an environment secret | Yes: repository hooks run in cloud sessions |
| Codex cloud | Same repo kit: `.codex/config.toml`, `AGENTS.md` | Environment secret | Verify MCP support in cloud tasks on the tested version; otherwise the CLI over the web API |
| Cursor cloud agents | `AGENTS.md` plus MCP (works today) | MCP OAuth | Instructed; cloud agents skip start/end hooks |
| Anything with a shell | `npx hypher resume` / `seal` over the web API | Environment secret | Instructed |

Rules for every door:

- **Cloud containers are fresh per session.** The repo kit is the install; per-machine setup does not apply there.
- **Git checks run where the code is.** Tree hash, ancestry, and "what changed since" run inside the container or on the laptop. Code never leaves it.
- **Network access is a setup step.** Cloud environments must allow `hypher.app`. When Hypher is unreachable, the kit says "Hypher unavailable; starting without it" instead of starting cold in silence. This review's own cloud session hit exactly that: the egress proxy returned 403.
- **Keys are scoped to one project.** A key placed in a cloud environment reaches one project, can be revoked on its own, and shows its last use. Today's API keys reach the whole account.
- **One package.** `hypher` is the bridge on a laptop and a plain command in a container. The kits are generated by `init` (user-level) and `init --repo` (committed), from one source.
...
| M3 | Flight recorder on Claude Code (weeks 3–4) | `hypher` bridge and outbox, Claude Code plugin hooks, `SessionStart` injection, seal on stop; `hypher init --repo` kit for cloud sessions; a "Hypher unavailable" notice | `kill -9` mid-task, resume in a new Claude Code session with zero commands; checkpoint age under 5 minutes |
...
| M5 | Install and approve (week 6) | `npx hypher init`, uninstall, keychain, elicitation, Pulse approve button, handoff card; project-scoped API keys for cloud environments | A pilot installs on their own repository in under 2 minutes and approves a decision without leaving the terminal |
