import Link from "next/link";

type MarketingBrandSize = "default" | "sm";

export function MarketingBrand({
  className = "",
  size = "default",
  showTagline = false,
}: {
  className?: string;
  size?: MarketingBrandSize;
  /** “Signal in motion” — brand line under wordmark (e.g. footer) */
  showTagline?: boolean;
}) {
  const markClass =
    size === "sm"
      ? "hypher-signal-mark hypher-signal-mark--marketing hypher-signal-mark--marketing-sm"
      : "hypher-signal-mark hypher-signal-mark--marketing";

  return (
    <Link
      href="/"
      className={`marketing-brand${size === "sm" ? " marketing-brand--sm" : ""}${className ? ` ${className}` : ""}`}
      aria-label="Hypher home"
    >
      <svg
        className={markClass}
        viewBox="0 0 596 151"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        aria-hidden
      >
        <path
          fill="currentColor"
          d="M0 10.5C0 8.29086 1.79086 6.5 4 6.5H179C181.209 6.5 183 8.29086 183 10.5V37C183 39.2091 184.791 41 187 41H367.5C369.709 41 371.5 39.2091 371.5 37V4C371.5 1.79086 373.291 0 375.5 0H551.014C552.785 0 554.345 1.16472 554.849 2.86275L595.477 139.863C596.237 142.427 594.316 145 591.642 145H413.17C412.406 145 411.658 144.781 411.015 144.37L369.985 118.13C369.342 117.719 368.594 117.5 367.83 117.5H187C184.791 117.5 183 119.291 183 121.5V146.5C183 148.709 181.209 150.5 179 150.5H4C1.79086 150.5 0 148.709 0 146.5V10.5Z"
        />
      </svg>
      <span className="marketing-brand__wordBlock">
        <span>hypher</span>
        {showTagline ? (
          <span className="marketing-brand__tagline" aria-hidden="true">
            Signal in motion
          </span>
        ) : null}
      </span>
    </Link>
  );
}
