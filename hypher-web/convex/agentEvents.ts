...
import { isWorkReceipt } from "../shared/silentMemory";
import { applyReceiptForEvent } from "./lib/silentMemoryWrite";
import { scheduleProjectMemorySynthesis } from "./lib/scheduleMemory";
...
async function persistAgentEventForUser(
  ...
  if (receipt && matched) {
    ...
    await ctx.scheduler.runAfter(0, _internal.projectMemoryActions.synthesize, {
      userId,
      projectId: matched.id,
      reason: "writeback",
    });
